...
while 1:
    ...
    if i > (3*(1/frequency)):
        ...
        # amplitude = random.uniform(0.35, 0.5)
    ...
    print("freq, amp:", frequency, amplitude)



    # print(frequency_L, '\n')
    # Calculate desired position and velocity using the current index ()
    # index = int((t_ - time_data[0]) / control_period)    # Calculate the index
    # print(t_, time_data[0], index, '\n')

    # if j>= 0 and j <= (sim_time/4):
    #     noise_stddev = 0.04 # 잡음의 강도, 표준 편차 (조절 가능)
    #     noise_stddev2 = 0.02
    #     noise1 = np.random.normal(0, noise_stddev)
    #     noise2 = np.random.normal(0, noise_stddev)
    #     noise3 = np.random.normal(0, noise_stddev2)
    #     shift = shift_ + noise3
    # elif j>(sim_time/2) and j <= (sim_time/4)*3 :
    #     noise_stddev = 0.04 # 잡음의 강도, 표준 편차 (조절 가능)
    #     noise_stddev2 = 0.02
    #     noise1 = np.random.normal(0, noise_stddev)
    #     noise2 = np.random.normal(0, noise_stddev)
    ...
    desired_pos_1 = calculate_desired_position(t_, frequency, amplitude, offset_L)+shift # output : rad
    desired_vel_1 = calculate_desired_velocity(t_, frequency, amplitude, offset_L)

    desired_pos_2 = calculate_desired_position(t_, frequency, amplitude, offset_R)+shift
    desired_vel_2 = calculate_desired_velocity(t_, frequency, amplitude, offset_R)

    if j >= (sim_time/2):
        if desired_pos_1 < shift:
            desired_pos_1 = calculate_desired_position(t_, frequency, amplitude* 2, offset_L)+shift
            desired_vel_1 = calculate_desired_velocity(t_, frequency, amplitude* 2, offset_L)
        else:
            desired_pos_2 = calculate_desired_position(t_, frequency, amplitude* 1.45, offset_R)+shift
            desired_vel_2 = calculate_desired_velocity(t_, frequency, amplitude* 1.45, offset_R)
    else:
        if desired_pos_1 < shift:
            desired_pos_1 = calculate_desired_position(t_, frequency, amplitude* 1.45, offset_L)+shift
            desired_vel_1 = calculate_desired_velocity(t_, frequency, amplitude* 1.45, offset_L)
        else:
            desired_pos_2 = calculate_desired_position(t_, frequency, amplitude* 2, offset_R)+shift
            desired_vel_2 = calculate_desired_velocity(t_, frequency, amplitude* 2, offset_R)

    

    # 가우시안 잡음을 desired_pos_1과 desired_pos_2에 추가
    desired_pos_1 += noise1
    desired_pos_2 += noise2


    print(desired_pos_1, desired_pos_2, shift, '\n')

    # desired_pos_1 = (left_position_data[i]) * DEG2RAD
    # desired_vel_1 = (left_velocity_data[i]) * DEG2RAD
    # desired_pos_2 = (right_position_data[i]) * DEG2RAD
    # desired_vel_2 = (right_velocity_data[i]) * DEG2RAD
    ...
    y = np.sin(desired_pos_2) - np.sin(desired_pos_1)

    # assist_torque_left  = k_t1 * y * (t_ - delta_t)
    # assist_torque_right = k_t2 * -y * (t_ - delta_t) 

    # 계산된 y(t)를 저장
    q_left.append(desired_pos_1)
    q_right.append(desired_pos_2)
    y_values.append(y)
    
    # τ(t) 계산
    if len(y_values) >= 0:
        
        tau_t = kappa * y # time delay : 0.25s , simulation period : 0.01s
        # tau_t = kappa * y_values[-26] # time delay : 0.25s , simulation period : 0.01s
        # tau_p = kappa * y_values[-1]
        # print("present", q_left[-1], q_right[-1], y_values[-1], tau_p)
        # print("before 25", q_left[-26], q_right[-26], y_values[-26], tau_t)

    else:
        tau_t = 0.0
        tau_p = 0.0
    tau_values.append(tau_t)
    # tau_pr.append(tau_p) # no delay
    ...
    data_to_write.append(f"{desired_pos_1*RAD2DEG} {desired_pos_2*RAD2DEG} {tau_t}") # left right torque
...
# output_file_path = '/home/kist/mujoco/mujopy_test_sindata1115.txt'
