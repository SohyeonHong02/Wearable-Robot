...
# left leg           |   right leg
frequency_L = 0.8    ;   frequency_R = 0.8
amplitude_L = 0.55    ;   amplitude_R = 0.55
...
while not glfw.window_should_close(window): 
    ...
    # print(data.time, time_data[0], index, '\n')
    
    desired_pos_1 = calculate_desired_position(data.time, frequency_L, amplitude_L, offset_L) # output : rad
    desired_vel_1 = calculate_desired_velocity(data.time, frequency_L, amplitude_L, offset_L)

    desired_pos_2 = calculate_desired_position(data.time, frequency_R, amplitude_R, offset_R)
    desired_vel_2 = calculate_desired_velocity(data.time, frequency_R, amplitude_R, offset_R)

    # print(desired_pos_1, desired_vel_1, '\n')

    # desired_pos_1 = (left_position_data[index]) * DEG2RAD
    # desired_vel_1 = (left_velocity_data[index]) * DEG2RAD
    # desired_pos_2 = (right_position_data[index]) * DEG2RAD
    ...
    # 계산된 y(t)를 저장
    q_left.append(data.qpos[0])
    q_right.append(data.qpos[2])
    y_values.append(y)
    ...
    data_to_write.append(f"{data.qpos[0]} {data.qpos[2]} {tau_t}")
...
# output_file_path = '/home/kist/mujoco/mujopy_train_nodelay1023.txt'
